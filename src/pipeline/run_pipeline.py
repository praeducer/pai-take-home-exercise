    ...
    for product in brief["products"]:
        ...
        for aspect_ratio, format_name in RATIO_FORMAT_MAP.items():
            try:
                ...
                if not dry_run:
                    # Save locally to outputs/results/
                    ...
                    if bucket:
                        ...
                        print(f"  + {product['name']} ({product.get('flavor', '')}) {aspect_ratio} -> {local_path}")
                else:
                    print(
                        f"  + {product['name']} ({product.get('flavor', '')}) {aspect_ratio} [dry-run, skipped]"
                    )
