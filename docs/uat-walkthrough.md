...
**What they are:** Versioned snapshots. v1.0.0 = exercise requirements met. v1.1.0 = image quality overhaul + SA document.

🔗 **v1.1.0 (current):** [https://github.com/praeducer/pai-take-home-exercise/releases/tag/v1.1.0](https://github.com/praeducer/pai-take-home-exercise/releases/tag/v1.1.0)
...
**Expected:** Both tags exist, v1.1.0 is the most recent.
...
- ✓ Lint with ruff
- ✓ Run unit tests (39 passed, 3 skipped)
...
☐ Local pytest shows 39 passed, 3 skipped
...
| `prompt_constructor.py` | Build format-specific (1:1 / 9:16 / 16:9) positive + negative prompts | `build_image_prompt()` → `tuple[str, str]` |
| `text_reasoning.py` | Claude Opus 4.6 brand profile (once/brief) + Sonnet 4.6 prompt enhancement | `generate_brand_profile()`, `enhance_prompt_with_reasoning()` |
...
**What it is:** 42 tests (39 pass, 3 skip, 1 integration deselected). Covers every pipeline module.
...
| `test_prompt_constructor.py` | Tuple return, format distinctness, brand_profile, negative prompt | ~11 |
...
| `test_text_reasoning.py` | Brand profile dry-run, default keys, no API call | ~3 |
...
# Expected: 39 passed, 3 skipped, 1 deselected
```
...
| `trail-mix-apac.json` | apac | Original, Matcha Green Tea | Zen minimalism, white + forest green, bamboo |
...
**What it is:** 471-line dual-audience SA document (engineers + business executives). Covers all 13 components, Well-Architected assessment, cost model, and production roadmap.
...
- Section 10: Cost Model — $2.10 PoC, ~$87/month at production scale

☐ Document renders in GitHub with all 15 section headers visible
...
**Expected output:**
```
outputs/demo/alpine-harvest-trail-mix/apac/front_label/matcha-green-tea.png
...
**What it is:** 24 generated PNG images organized by `{sku_id}/{region}/{format}/{product}.png`, plus 8 run manifests.
...
**Expected counts:** 24 PNG files, 8 JSON manifest files
...
| `anthropic.claude-sonnet-4-6` | Prompt enhancement (per image) | ACTIVE | ~$0.001/call |
| `anthropic.claude-opus-4-6-v1` | Brand profile generation (per brief) | ACTIVE | ~$0.01/call |
...
| **Multi-step AI chain** — Opus for brand direction, Sonnet for prompt refinement, Nova Canvas for generation | `text_reasoning.py` + `prompt_constructor.py` source; Section 5 of SA doc |
...
| "How does it ensure visual consistency across the 6 images?" | `generate_brand_profile()` calls Claude Opus once per brief to establish color palette, photography style, and regional visual elements. All 6 images inherit that profile. |
